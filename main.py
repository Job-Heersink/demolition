...
# my_system.SetSolverMaxIterations(70) #less precise, faster

# Create a contact material (surface property)to share between all objects.
# The rolling and spinning parameters are optional - if enabled they double
# the computational time.
building_material = chrono.ChMaterialSurfaceNSC()
building_material.SetFriction(0.5)
building_material.SetDampingF(0.2)
building_material.SetCompliance(0.0000001)
building_material.SetComplianceT(0.0000001)
building_material.SetRollingFriction(0.0000002)  # TODO: does this work right?
building_material.SetSpinningFriction(0)
building_material.SetComplianceRolling(0.0000001)
building_material.SetComplianceSpinning(0.0000001)
...
inertia_brick = 2 / 5 * (pow(size_brick_x, 2)) * mass_brick  # TODO: compute separate xx,yy,zz inertias
...
for ix in range(0, nbricks_on_x):
    for iy in range(0, nbricks_on_y):
        ...
        body_brick.GetCollisionModel().AddBox(building_material, size_brick_x / 2, size_brick_y / 2,
...
body_floor.GetCollisionModel().AddBox(building_material, 3, 1, 3) # hemi sizes
...
body_floor_texture.SetTextureFilename(chrono.GetChronoDataFile(r'textures/concrete.jpg'))
...
body_table.GetCollisionModel().AddBox(building_material, size_table_x/2, size_table_y/2, size_table_z/2) # hemi sizes
